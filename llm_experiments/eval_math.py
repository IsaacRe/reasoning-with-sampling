...
def eval_math(fname):
    ...
    temp_correct = 0
    mcmc_correct = 0
    tree_search_correct = 0
    total = len(df)

    tree_search_results = df.get("tree_search_answer", None)
    ...
    for i in range(total):
        ...
        if tree_search_results is not None:
            tree_search_correct += safe_grade(tree_search_results[i], df["correct_answer"][i])


    return base_correct, temp_correct, mcmc_correct, tree_search_correct, total
...
def math_results(fnames):
    ...
    for fname in fnames:
        base, temp, mcmc, ts, n = eval_math(fname)
        base_total += base
        temp_total += temp
        mcmc_total += mcmc
        ts_total += ts
    ...
    print(f"Tree Search accuracy:  {ts_acc:.3f}")
